...
import com.yc.compare.api.WeatherServiceApi;
import com.yc.compare.util.NetUtil;
...
public class RetrofitManager1 {
    ...
    public static final String CACHE_CONTROL_CACHE = "only-if-cached, max-stale=" + CACHE_STALE_SHORT;
    //查询网络的Cache-Control设置，头部Cache-Control设为max-age=0时则不会使用缓存而请求服务器
    public static final String CACHE_CONTROL_NETWORK = "max-age=0";
    private static OkHttpClient mOkHttpClient;
    private final WeatherServiceApi weatherServiceApi;
    private final HomeDataServiceApi homeDataServiceApi;

    public static RetrofitManager1 builder() {
        return new RetrofitManager1();
    }
    ...
    private RetrofitManager1() {
        ...
        Retrofit retrofit = new Retrofit.Builder()
                .baseUrl(Constants.HOME_DATA_URL)
                .client(mOkHttpClient)
                .addCallAdapterFactory(RxJavaCallAdapterFactory.create())
                .addConverterFactory(GsonConverterFactory.create())
                .build();
        weatherServiceApi = retrofit.create(WeatherServiceApi.class);
        homeDataServiceApi = retrofit.create(HomeDataServiceApi.class);
    }
